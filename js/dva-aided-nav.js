...
;(function($){
	...
	$.fn.superfish = function(op){
		...
		var sf = $.fn.superfish,
			...
			out = function(){
				...
				menu.sfTimer=setTimeout(function(){
					...
					if (o.$path.length && $this.parents(['li.aided-nav-li',o.hoverClass].join('')).length<1){over.call(o.$path);}
				},o.delay);	
			},
			getMenu = function($menu){
				var menu = $menu.parents(['ul.',c.menuClass,':first'].join(''))[0];
				//var menu = $menu.parents(['ul.',c.menuClass,':first'].join(''))[0];
				//if ( $( this ).hasClass( "sf-menu-aided-navigation" ) ) {
		...
		return this.each(function() {
			...
			o.$path = $('li.aided-nav-li'+o.pathClass,this).slice(0,o.pathLevels).each(function(){
				$(this).addClass([o.hoverClass,c.bcClass].join(' '))
					.filter('li.aided-nav-li:has(ul)').removeClass(o.pathClass);
			});
			sf.o[s] = sf.op = o;
      // CHANGED: by KARL SWEDBERG
			...
			$('li.aided-nav-li:has(ul)',this)[o.eventType](over,out).each(function() {
				if (o.autoArrows) {
			...
			var $a = $('a',this);
			$a.each(function(i){
				var $li = $a.eq(i).parents('li.aided-nav-li');
	...
	sf.defaults = {
		...
		dropShadows : true,
    // CHANGED: by KARL SWEDBERG
		eventType   : 'toggle', // one of 'toggle', 'hover', or 'hoverIntent' this is the click function
	...
	$.fn.extend({
		hideSuperfishUl : function(){
			...
			o.retainPath = false;
			var $closingLi = $(['li.aided-nav-li',o.hoverClass].join(''),this).add(this).not(not);
			var $ul = $closingLi
					.find('ul');
